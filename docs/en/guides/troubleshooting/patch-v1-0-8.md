---
title: Beast of Reincarnation Patch v1.0.8 (v1.006.000) Notes
description: "Source-checked Beast of Reincarnation v1.0.8 patch notes: DLSS, FSR, camera, subtitles, fixes, and upcoming features."
---
# Beast of Reincarnation Patch v1.0.8 (v1.006.000)

<div class="meta"><span>Published: Aug 16, 2026</span><span>Official patch summary</span><span class="verified">Based on Fictions’ v1.0.8 / v1.006.000 notes</span></div>

<div class="quick-answer"><strong>Quick Answer</strong>Patch <strong>v1.0.8 / v1.006.000</strong> is live. It adds PC DLSS 4/4.5 (L/M) and FSR4 support, camera-distance and subtitle-size options, freer Interact-button mapping, plus selected performance, gameplay, camera, cutscene, and audio fixes.</div>

## Main additions

- **Camera distance:** choose Close, Medium, or Far.
- **Subtitles:** choose Extra Small, Small, Medium, Large, or Extra Large. Fictions advises against setting gameplay subtitles above Medium to avoid UI overlap.
- **Input:** Interact can be assigned to any button or key; PC players can assign it to side mouse buttons.
- **PC graphics:** DLSS 4/4.5 (L/M) and FSR4 are listed in the official notes.

## Selected fixes and adjustments

The official notes describe adjustments to player and cinematic cameras, event/cutscene triggers, selected enemy placement and boss behavior, and audio/video noise in certain cutscenes. They also identify a fix for delayed enemy spawning in cloud-streaming areas and a severe frame drop when activating Kiku's “End of the Line” Bloom Strike.

The notes also change the default auto-target setting to off for new configurations. Existing configurations are not changed automatically.

## Still upcoming

Fictions lists frame generation, incremental event/cutscene skipping, and ultrawide support as future work. It says ultrawide is in final testing and will be released soon, but gives no exact date.

## Need help after updating?

- [No DLSS option on PC Game Pass?](/en/guides/troubleshooting/ultrawide-dlss-fsr-status)
- [Controller stopped working or input conflict](/en/guides/troubleshooting/controller-settings)
- [Known issues and patch status](/en/guides/known-issues-patch-status)
- [Fictions Support](https://support.fictions.com/hc/en-us/categories/42992376012691-Beast-of-Reincarnation)

## Official source

- [Fictions: Patch Notes v1.0.8 / v1.006.000](https://support.fictions.com/hc/en-us/articles/54430411510419--BOR-Patch-Notes-v1-0-8-v1-006-000)
